...
class Fetcher {
  ...
  linkData(originalUri, rel, uri, why, reverse) {
    ...
    if (rel === 'alternate' || rel === 'seeAlso' || rel === 'meta' || rel === 'describedby') {
      ...
      predicates = [kb.rdfFactory.namedNode(this.ianaLinkRelation(rel)), this.ns.rdfs('seeAlso')];
    } else if (rel === 'type') {
      ...
      predicates = [kb.rdfFactory.namedNode(this.ianaLinkRelation(rel))];
    }
  ...
  ianaLinkRelation(rel) {
    return Uri.join(encodeURIComponent(rel), 'http://www.iana.org/assignments/link-relations/');
  }
